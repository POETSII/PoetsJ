...
public class DPhilIntRun {
	...
	/*public static void printTtrack() {
		for(int i=0; i<transitions; i++) {
			int e = DPhilDevice.ttrackEnabled[i];
			int f = DPhilDevice.ttrackNotFired[i];
	...
	public static void main(String[] args) {
		PipelineTransitionMap.create(1);
		//DPhilDevice.ttrackEnabled = new int[transitions];
		//DPhilDevice.ttrackNotFired = new int[transitions];
		...
		if(display) {
			//UIWindow frame = SwingWindowFactory.use(1f).createFrame("DPhil", 800, 800);
			final SwingFrame frame = new SwingFrame(SwingWindowFactory.use(1f), "DPhil", 800, 800, true, false) {
				@Override
